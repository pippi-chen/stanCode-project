...
def add_data_for_name(name_data, year, rank, name):
    # Set the situation that the name is in the dataset.
    if name not in name_data:
        name_data[name] = {year: rank}

    # Set the situation that the name is not in the dataset.
    else:
        if year in name_data[name]:
            # The situation we found there is a lower rank with the same name in the same year within the dataset.
